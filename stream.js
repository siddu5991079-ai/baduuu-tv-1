...
async function takeAndBatchScreenshot(page, stepName) {
    try {
        ...
        if (pendingScreenshots.length >= 3) {
            // 👉 DEBUGGING UPDATE: Upload cycle limit logic commented out for 24/7 logging
            /*
            if (uploadCycleCount >= 4) {
                console.log(`[!] Upload cycle limit (4) reached. Skipping GitHub Release upload.`);
                pendingScreenshots = [];
                return;
            }
            */
...
async function startDirectStreaming() {
    ...
    console.log('[*http://googleusercontent.com/image_generation_content/0
...
//                 uploadCycleCount++;
//                 console.log(`[+] Live batch upload successful! (Cycle ${uploadCycleCount}/4) Aap ab Github Release check kar sakte hain.`);
//                 pendingScreenshots = []; 
//             } catch (err) {
//                 console.log(`[-] Live upload failed: ${err.message}`);
//             }
...
//         // 👉 A/V SYNC FIX: Value 0.8 se barha kar 1.5 kar di hai!
...
//                     if (v && !v.ended) return 'HEALTHY';
//                     return 'DEAD';
//                 });

//                 if (status === 'CRITICAL_ERROR') criticalErrorFound = true;
//                 if (status === 'HEALTHY') overallStatus = 'HEALTHY'; 
//             } catch (e) {}
//         }

//         if (criticalErrorFound) overallStatus = 'CRITICAL_ERROR';

//         if (overallStatus === 'CRITICAL_ERROR' || overallStatus === 'DEAD') {
...
//             await takeAndBatchScreenshot(page, `health-check-${watchdogTicks}`);
//         }
